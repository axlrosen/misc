...
class Puzzle:
    ...
    def reset(self):
        self.fill = self.orig_fill
        self.added = set()

    def to_grid(self, loc):
        return loc[0] + self.width * loc[1]

    def fill_across(self, loc, entry):
        loc = loc.copy()
        for c in entry:
            if loc[0] == 15:
                raise ValueError
            i = self.to_grid(loc)
            self.fill = self.fill[:i] + c + self.fill[i + 1:]
            loc[0] += 1
        self.added.add(entry)

    def fill_down(self, loc, entry):
        loc = loc.copy()
        for c in entry:
            if loc[1] == 15:
                raise ValueError
